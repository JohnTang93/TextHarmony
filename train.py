...
def main():
    ...
    if getattr(config, "load_from", None):
        print('*'*40, 'load checkpoints', '*'*40)
        load_model_weights(trainer.model, config.load_from, 
            image_upscale=config.image_upscale, use_lora=config.use_lora,
            vit_lora=False if config.model.get('moe_config', None) is None else config.model.moe_config.get('vit_lora', True))
